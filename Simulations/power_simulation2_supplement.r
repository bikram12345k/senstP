...
#for(itr in 1:Itr){
	...
	### Choose
	## t-model for outcome difference or 
	##	Gaussian model for outcomes difference
	R1 = rt(I, 3)/sqrt(6)
	R2 = theta[gp] + rt(I, 3)/sqrt(6)
	...
	combinedPvals_allgam <- foreach(gammaID = 1:length(gamSeq),
		...
		n = length(pvals)
		#for(u in 1:3){#(n-1)){
		combinedPvals = foreach(u=1:2, .combine = 'cbind', 
	...
	Simes[itr, 1:2, 1:length(gamSeq)] = matrix(combinedPvals_allgam[1,], ncol=length(gamSeq))
	Fisher[itr, 1:2, 1:length(gamSeq)] = matrix(combinedPvals_allgam[2,], ncol=length(gamSeq))
	tP[itr, 1:2, 1:length(gamSeq)] = matrix(combinedPvals_allgam[3,], ncol=length(gamSeq))
	SumP[itr, 1:2, 1:length(gamSeq)] = matrix(combinedPvals_allgam[4,], ncol=length(gamSeq))
	
	stPI[itr, 1:2, 1:length(gamSeq)] =  matrix(combinedPvals_allgam[5,], ncol=length(gamSeq))
	stPII[itr, 1:2, 1:length(gamSeq)] =  matrix(combinedPvals_allgam[6,], ncol=length(gamSeq))
	stPIII[itr, 1:2, 1:length(gamSeq)] =  matrix(combinedPvals_allgam[7,], ncol=length(gamSeq))
	stPIV[itr, 1:2, 1:length(gamSeq)] =  matrix(combinedPvals_allgam[8,], ncol=length(gamSeq))
	stPV[itr, 1:2, 1:length(gamSeq)] =  matrix(combinedPvals_allgam[9,], ncol=length(gamSeq))
	stPVI[itr, 1:2, 1:length(gamSeq)] =  matrix(combinedPvals_allgam[10,], ncol=length(gamSeq))
	
	stPIf[itr, 1:2, 1:length(gamSeq)] =  matrix(combinedPvals_allgam[11,], ncol=length(gamSeq))
	stPIIf[itr, 1:2, 1:length(gamSeq)] =  matrix(combinedPvals_allgam[12,], ncol=length(gamSeq))
	stPIIIf[itr, 1:2, 1:length(gamSeq)] =  matrix(combinedPvals_allgam[13,], ncol=length(gamSeq))
	stPIVf[itr, 1:2, 1:length(gamSeq)] =  matrix(combinedPvals_allgam[14,], ncol=length(gamSeq))
	stPVf[itr, 1:2, 1:length(gamSeq)] =  matrix(combinedPvals_allgam[15,], ncol=length(gamSeq))
	stPVIf[itr, 1:2, 1:length(gamSeq)] = matrix(combinedPvals_allgam[16,], ncol=length(gamSeq))
